...
- The server emits a frame only when display content changes: static screens legitimately yield 0–3 fps, and the receive rate under interaction tracked a raw-socket control client reading the same stream (~3–5 fps on a software encoder under host CPU load), while an accumulated backlog drained at 32 fps — the decode/UI pipeline was not the limiter.
