...
Implementation must use public specifications or fixtures captured by Fadb.
...
- Pinned server artifact `scrcpy-server-v3.3.4` (90,980 bytes), SHA-256 `8588238c9a5a00aa542906b6ec7e6d5541d9ffb9b5d0f6e1bc0e365e2303079e`, downloaded from https://github.com/Genymobile/scrcpy/releases/download/v3.3.4/scrcpy-server-v3.3.4 (release list cross-checked via the GitHub API `digest` field for release 271207312). The artifact is vendored at `crates/fadb-scrcpy/assets/scrcpy-server-v3.3.4`; Fadb verifies the SHA-256 at build time via `include_bytes!` + a unit test. scrcpy is Apache-2.0 licensed; this artifact is redistributed unmodified with attribution here.
...
- The default `cleanup=true` server removes `/data/local/tmp/scrcpy-server.jar` when the server process exits (observed: jar present at push, gone after the server was killed), so Fadb re-pushes the jar on every mirror start.
