...
/// Editable copy of [`bridgescope_domain::AiSettings`] backing the settings
...
impl AiSettingsForm {
    #[must_use]
    pub fn from_settings(settings: Option<&bridgescope_domain::AiSettings>) -> Self {
    ...
    /// Normalize into [`bridgescope_domain::AiSettings`] when endpoint and
    /// model are filled. An empty or unparsable timeout falls back to 30 s.
    #[must_use]
    pub fn to_settings(&self) -> Option<bridgescope_domain::AiSettings> {
        ...
        let timeout_seconds = self
            ...
            .unwrap_or(30)
            .clamp(1, 600);
        Some(bridgescope_domain::AiSettings {
            endpoint,
            model,
...
impl AssistantPanelState {
    ...
    /// `BRIDGESCOPE_ASSISTANT=2` in `app.rs`).
...
mod tests {
    ...
    fn ai_settings_form_roundtrips_settings() {
        let settings = bridgescope_domain::AiSettings {
