...
use bridgescope_domain::{BackendCommand, BackendEvent, DeviceTarget, LogcatSessionId};
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button(text(language, "logcat_save")).clicked()
            && let Some(path) = rfd::FileDialog::new()
                .set_file_name("bridgescope-logcat.txt")
...
mod tests {
    ...
    fn auto_start_requires_online_target_and_backs_off() {
        ...
        let target = DeviceTarget::new(
            bridgescope_domain::DeviceSerial::new("emulator-5554").expect("serial"),
            1,
        );
