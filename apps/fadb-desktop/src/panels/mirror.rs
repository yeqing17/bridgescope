...
use fadb_domain::{BackendCommand, BackendEvent, BridgeError, DeviceTarget, OperationId};
use fadb_scrcpy::ScrcpySessionPlan;
...
pub struct MirrorPanelState {
    ...
    /// Throttle for the `FADB_MIRROR_RECORD` dev hook.
...
fn absorb_frames(
    ...
    if let Some(frame) = frame {
        ...
        if let Some(texture) = state.texture.as_mut() {
            ...
            state.texture =
                Some(context.load_texture("fadb-mirror", image, egui::TextureOptions::LINEAR));
        }
        let now = Instant::now();
...
/// Dev hook (`FADB_MIRROR_AUTO=1`): starts mirroring automatically so
...
pub fn auto(state: &mut MirrorPanelState, selected: Option<&DeviceTarget>) -> Vec<BackendCommand> {
    ...
    if std::env::var_os("FADB_MIRROR_AUTO").is_some()
        && idle
        && backed_off
        && let Some(target) = selected
    {
    ...
    // Dev hook (`FADB_MIRROR_RECORD=<secs>`): records the running
    ...
    if !state.record_auto_done
        && let Some(secs) = std::env::var_os("FADB_MIRROR_RECORD")
...
mod tests {
    use super::*;
    use fadb_domain::{DeviceSerial, ErrorCode};
