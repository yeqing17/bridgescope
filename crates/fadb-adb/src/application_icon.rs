...
use fadb_domain::{ApplicationIconData, BridgeError, DeviceSerial, ErrorCode, PackageName};
...
mod tests {
    ...
    /// Manual probe against a live device — set `FADB_ICON_PROBE` to
    /// the serial, `FADB_ICON_PACKAGE` to a package (optionally
    /// `FADB_ADB` to the executable) and run
    /// `cargo test -p fadb-adb -- --ignored probe_real_device_icon`.
    /// Decodes are dumped to the temp dir for inspection.
    #[tokio::test]
    #[ignore = "needs a live device; drive it with FADB_ICON_PROBE"]
    async fn probe_real_device_icon() {
        let Some(serial) = std::env::var_os("FADB_ICON_PROBE") else {
            return;
        };
        let package = std::env::var("FADB_ICON_PACKAGE").expect("package env set");
        let executable = std::env::var_os("FADB_ADB")
            .map_or_else(|| std::path::PathBuf::from("adb"), std::path::PathBuf::from);
        let serial = fadb_domain::DeviceSerial::new(serial.to_string_lossy().into_owned())
            .expect("valid serial");
        let package = fadb_domain::PackageName::new(&package).expect("valid package");
        ...
        let path = std::env::temp_dir().join("fadb-icon-probe.png");
