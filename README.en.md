...
- **File manager:** browses remote directories, uploads and downloads files with explicit overwrite confirmation, supports cancellation, creates directories, renames entries, and deletes files and directories (recursive directory deletion, with explicit confirmation). The listing shows modification times and sorts by name/size/modification time. Operations remain bound to the selected device generation and refresh the current listing after completion.
...
- **Network devices:** the Device Manager accepts an Android device host/IP and port directly through adb connect, keeps up to eight successful endpoints as local history, allows reconnecting or forgetting saved endpoints, and disconnects connected network devices with one click. Device discovery runs once at startup and again only after an explicit refresh or network connection.
